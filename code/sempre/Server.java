...
public class Server {
  ...
  class ExchangeState {
    ...
    public ExchangeState(HttpExchange exchange) throws IOException {
      ...
      String sessionId = reqParams.get("sessionId");
      ...
      if (value instanceof NameValue) {
        ...
        for (List<Value> rowValues : ((TableValue) value).rows) {
          ...
          for (Value x : rowValues) {
            // TODO(): add horizontal spacing only using CSS
    ...
    private void markLexical(Derivation deriv, CandidatePredicates[] predicates) {
      // TODO(): generalize this to the case where the formula is a
    ...
    String makeJson(Master.Response response, String sessionId) {
      ...
      if (response != null) {
        ...
        for (Derivation deriv : response.getExample().getPredDerivations()) {
          ...
          if (value instanceof UriValue) {
            ...
            item.put("formula", deriv.formula);
      ...
      if (masterResponse != null) {
        ...
        if (ex != null) {
          if (!jsonFormat()) {
            ...
            out.println(makeJson(masterResponse, sessionId));
          }
        } else if (jsonFormat()) {
          out.println(makeJson(null, sessionId));
        }
